...
while ! curl --silent https://netbox.admin.it.airnity.private/api/schema/?format=yaml > api/openapi.yaml 2> /dev/null; do
# while ! curl --silent https://netbox.netbox-airnity-plugin.orb.local/api/schema/?format=yaml > api/openapi.yaml 2> /dev/null; do
  sleep 1
done
